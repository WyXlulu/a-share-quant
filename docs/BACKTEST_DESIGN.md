...
1. 本规范实际文件名为 `BACKTEST_DESIGN.md`，正文中 `BACKTEST_DESIGN_V3_0_2.md` 的自引用以此为准。
2. `src` 下 `calendar` 包已更名为 `market_calendar`（避 Python 标准库命名冲突，见 `DECISIONS.md` 2026-07-01），正文 §2.2 目录树中 `calendar/` 以此为准。
3. §0.5要求黄金切片L2台账“手工建立并双人核验”。实际执行为单人核验+AI复核:由项目负责人逐份打开巨潮官方PDF原件读取字段并签字(`verified_by`),由Claude按官方参考价公式复算、核对PIT时序与条数查漏。此为对“双人核验”的偏离,如实记录,不视为已满足双人核验要求。
